...
fn main() {
    ...
    let partitioned = partition_data(num_partitions, &v);
    // 2. Calls print_partition_info to print info on the partitions that have been created
    print_partition_info(&partitioned);
    // 3. Creates one thread per partition and uses each thread to concurrently process one partition
    let mut threadvector : Vec<JoinHandle<usize>> = Vec::new();
   
    for i in partitioned{
      let spawnthread = thread::spawn(move || map_data(&i));

      threadvector.push(spawnthread);
    }
    // 4. Collects the intermediate sums from all the threads
    let mut CollectImSum : Vec<usize> = Vec::new();
    
    for i in threadvector{
      let thread2 = i.join().unwrap();

      CollectImSum.push(thread2);
    }
    // 5. Prints information about the intermediate sums
    println!("Intermediate sums = {:?}", CollectImSum);
    // 6. Calls reduce_data to process the intermediate sums
    let ImSumProcess = reduce_data(&CollectImSum);
    // 7. Prints the final sum computed by reduce_data
    println!("Sum = {}", ImSumProcess);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    //Get the size of each parititon
    let partition_size = v.len() / num_partitions;
    
    let numelements = v.len();

    let mut xs: Vec<Vec<usize>> = Vec::new();

    //Mutliple check
    let mut checkremainder = v.len() % num_partitions;
    ...
    for i in 0..num_partitions{
      let mut x1: Vec<usize> = Vec::new();
      //Set our partition size at each index in this case even
      for j in 0..partition_size{
        x1.push(v[index]);
        index = index + 1;
      }
      //Odd paritions if not a multiple
       if checkremainder != 0{
        x1.push(v[index]);
        index = index + 1;
        checkremainder = checkremainder - 1;
      }
