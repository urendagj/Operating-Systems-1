...
** * Input: Gets the message from the command line that was sent by the client
** * Output: Encrypted message is returned to client
**********************************************************************/
...
/*This function is what the server will use to encrypt the message that was send by the user
* This function will encode a message to be someting similar to: DSFGG DGFDGFD GFD GFDDDFFGD
* We also perform error checking to see if we have valid input 'A' - 'Z', if not we return an error
*/
char* encrypt(char* message, char* key) {
	char n;
	int i;
	int j;
	//Get the length of our plaintext file
	char messagelen = strlen(message);
	for (j = 0; j < messagelen; j++) {
		//check to see if we are within the range of our allowed input
		if (message[i] < 'A' || message[i] > 'Z' && message[i] != ' ')                     // Make sure that all the characters are valid
		{
			//throw an error when invalid input is found
